...
<meta name="decorator" content="task-form" />
		...
		<form:form method="POST" commandName="taskform" cssClass="form form-horizontal">
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						<span class="control-text">${taskform.name }</span>
					</div>
				</div>
											...
											<div class="remediation-actions pull-right span6">
												...
												<c:if test="${r.canAccept }">
													<label class="radio text-success">
													  ...
													  <spring:message text="${def }" code="${approveActionKey }" htmlEscape="false" arguments="${r.fieldTitle },${r.actualValue },${r.expectedValue },${r.sourceSystem }"/>
												</label>
												</c:if>
												...
												<c:if test="${r.canSuggest }">
												<label class="radio">
												  <form:radiobutton path="exceptions[${s.index }].action" value="suggest"/>
													  	<c:set var="def">
														  I'd like to specify a new value for '${r.fieldTitle }' 
														</c:set>
												  		<spring:message text="${def }" code="${r.suggestActionKey }" htmlEscape="false" arguments="${r.fieldTitle },${r.actualValue },${r.expectedValue },${r.sourceSystem }"/>
												  		<form:input path="exceptions[${s.index }].userSuggestedValue" />
												</label>
												</c:if>
